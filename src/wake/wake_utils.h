...
inline void KERNEL(	const double &rho, 
	...
	double volume = sigma*sigma*sigma;
	double phi = 0.25*M_1_PI*erf(M_SQRT1_2*rho_bar)/volume;
	Z = 0.5*exp(-0.5*rho_bar*rho_bar)/volume/pow(M_PI,1.5);
...
/*! \fn inline void RETVORCITY(const double &q, const double &F, const gsl_vector *source_vorticity, const gsl_vector *target_vorticity, const gsl_vector *displacement, gsl_vector *retvorcity )
 ...
 */
inline void RETVORCITY(	const double &q, 
			const double &F, 
...
inline void INTERACT(	const double &nu,
	...
	RETVORCITY(q,F,a_source,a_target,displacement,da);
